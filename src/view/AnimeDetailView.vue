    ...
    <div class="content-container box">
        <div class="video-container">
            <video :src="state.currentPlay.url" controls>
                <track :src="state.subtitle" kind="subtitles" default>
            </video>
            <div class="episode-container">
                <div class="horiz-warp">
                    <div class="episode-item box center" v-for="(item,index) in state.fileList.filter(
                            fileListItem=>state.fileTypeList.video.includes(fileListItem['fileType'])
        ...
        <div class="introduction">
            ...
            <div>
                <div class="title">标题：{{state.title}}</div>
                <div class="title">简介：{{state.info}}</div>
            </div>
        </div>
        <hr/>
    </div>
...
<script setup lang="ts">
    ...
    async function collect() {
        ...
        let resData: I_ResData<null> = await api.addCollectApi(animeUuid) as unknown as I_ResData<null>;
        if (resData.code === 200) {
    ...
    async function init() {
        ...
        let resData: I_ResData<I_DetailAndTag> = await api.animePostApi(uuid) as unknown as I_ResData<I_DetailAndTag>;
        state.fileList = resData.data.detail.fileList;
        state.title = resData.data.detail.title;
        state.info = resData.data.detail.info;
        ...
        if (![undefined, null, "null", "undefined", ""].includes(localStorage.getItem("token"))) {
            let reData:I_ResData<boolean> = await api.collectIsHaveApi(uuid) as unknown as I_ResData<boolean>;
            if (reData.data) {
                state.collectInfo.text = "已收藏";
                state.collectInfo.status = true;
            } else {
