<template>
    <div class="gui-container">
        ...
        <div class="role-gui-container" style="justify-content: flex-end;">
            <div v-show="detectShow">按E互动</div>
            <h1 style="background: rgba(0,0,0,0.5)">{{ subtitle }}</h1>
            <h2>{{ activeInfo }}</h2>
            <table style="border-collapse: collapse">
                ...
                <tr>
                  <td v-for="index in 10" :key="index"
                      ...
                      :style="[roleGoodsIndex===index?'background:orangered':'background:']">
                    <div style="width: 100%;height: 100%;display: flex;justify-content: center;align-items: center;"
                         v-if="index <= roleGoods.length">
                      {{ roleGoods[index - 1]?.userData?.type }}
                    </div>
                  </td>
                </tr>
                </tbody>
        ...
        <div class="itemInfo-container" style="justify-content: flex-end;align-items: flex-start;">
            <h2 v-for="(info) in roleGoods[roleGoodsIndex - 1].userData.infoList"
                 :key="info"
