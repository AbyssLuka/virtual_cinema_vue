<template>
    <div class="document-content" id="document-content">
        <div v-for="(item ,index) in fileInfoLineList" :key="index"
...
import {reactive, defineProps, watch, withDefaults, ref} from "vue";
import {I_File} from "@/global/interface";

const fileInfoLineList = ref<string[]>([]);
...
const openDocument = (docObj: I_File) => {
    let fileInfo = util.loadFile(baseUrl + "/file/" + docObj["fileUuid"]);
    if (fileInfo != null) {
        fileInfoLineList.value = fileInfo.split("\n");
    }
}
</script>
