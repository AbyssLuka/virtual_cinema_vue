...
<script setup lang="ts">
    ...
    function updateOrangeBar() {
        videoPlayer.value.addEventListener("timeupdate", () => {
            //视频长度
            let duration = videoPlayer.value.duration;
            //视频进度
            ...
            //缓冲段
            let buffereds = videoPlayer.value.buffered;
    ...
    async function animeList(videoUuid: string) {
        const resData: I_ResData<I_Video> = (await api.videoApi(videoUuid)) as unknown as I_ResData<I_Video>;
        playVideo(resData.data);
    }
